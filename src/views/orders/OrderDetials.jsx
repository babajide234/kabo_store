...
import { Grid, List, ListItem, Typography, Box, CircularProgress } from '@mui/material'
        ...
        <Grid container spacing={7}>
          ...
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Typography variant='caption'>{order?.store[0] ? order?.store[0].name : ''}</Typography>
            </Box>
          </Grid>
          <Grid item xs={12} mt={20}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Typography variant='caption'>
                {order?.customer[0] ? order?.customer[0].othernames : ''}{' '}
                {order?.customer[0] ? order?.customer[0].lastname : ''}
              </Typography>
            </Box>
          </Grid>
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Typography variant='caption'>{order?.customer[0] ? order?.customer[0].phone : ''}</Typography>
            </Box>
          </Grid>
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Typography variant=''>Address:</Typography>
              <Typography variant='caption'>{order?.store[0] ? order?.store[0].address : ''}</Typography>
            </Box>
          </Grid>
